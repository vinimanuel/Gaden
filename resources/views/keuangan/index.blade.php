...
<div class="stats-grid" style="margin-bottom:20px">
  <div class="stat-card">
    ...
    <div>
      ...
      <div class="stat-value blue">{{ rupiah($totalPinjaman) }}</div>
  ...
  <div class="stat-card">
    ...
    <div>
      ...
      <div class="stat-value green">{{ rupiah($totalBunga) }}</div>
      <div class="stat-sub">{{ $logs->where('tipe','Tebus')->count() }} penebusan</div>
    </div>
  </div>
  <div class="stat-card">
    ...
    <div>
      ...
      <div class="stat-value amber">{{ rupiah($totalPerp) }}</div>
      <div class="stat-sub">{{ $logs->where('tipe','Perpanjang')->count() }} perpanjangan</div>
    </div>
  </div>
  <div class="stat-card" style="border-left:4px solid var(--green)">
    ...
    <div>
      ...
      <div class="stat-value green">{{ rupiah($totalPendapatan) }}</div>
      <div class="stat-sub">bunga + perpanjangan</div>
    </div>
  </div>
</div>

{{-- Baris Potensi & Outstanding --}}
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:24px">
  <div class="card">
    <div class="card-body" style="padding:16px 20px">
      ...
      <div style="font-size:22px;font-weight:800;color:var(--blue)">{{ rupiah($potensiNormal) }}</div>
      <div style="margin-top:6px;font-size:11px;color:var(--text-3)">Jika ditebus bunga khusus (5%)</div>
      <div style="font-size:18px;font-weight:700;color:var(--green);margin-top:3px">{{ rupiah($potensiKhusus) }}</div>
    </div>
  </div>
  <div class="card">
    <div class="card-body" style="padding:16px 20px">
      ...
      <div style="font-size:22px;font-weight:800;color:var(--amber)">{{ rupiah($potensiPerp) }}</div>
    </div>
  </div>
  <div class="card">
    <div class="card-body" style="padding:16px 20px">
      ...
      <div style="font-size:22px;font-weight:800;color:var(--red)">{{ rupiah($outstanding) }}</div>
      <div style="margin-top:6px;font-size:11px;color:var(--text-3)">{{ $aktif->count() }} barang aktif</div>
    </div>
  </div>
</div>
...
<div class="card">
  <div class="card-header">
    ...
    <div style="font-size:13px;color:var(--text-3)">{{ $logs->count() }} transaksi</div>
  </div>
  <div class="table-wrap">
    <table>
      ...
      <tbody>
        @php $tc = ['Tebus'=>'b-green','Perpanjang'=>'b-amber']; @endphp
        @forelse($logs as $log)
          <tr>
            <td style="font-size:13px;color:var(--text-3);white-space:nowrap">{{ $log->tgl_transaksi->isoFormat('D MMM YYYY') }}</td>
            <td><span class="badge {{ $tc[$log->tipe] ?? 'b-blue' }}">{{ $log->tipe }}</span></td>
            <td><a href="{{ route('barang.show', $log->barang_gadai_id) }}" class="td-id" style="text-decoration:none">{{ $log->kode_gadai }}</a></td>
            <td class="td-name">{{ $log->nasabah }}</td>
            <td style="color:var(--text-2)">{{ $log->barang }}</td>
            <td class="td-money">{{ rupiah($log->nilai_pinjaman) }}</td>
            <td>
              <span style="font-size:15px;font-weight:800;color:{{ $log->tipe==='Tebus' ? 'var(--green)' : 'var(--amber)' }}">
                + {{ rupiah($log->biaya) }}
              </span>
            </td>
      ...
      @if($logs->count())
        <tfoot>
          <tr style="background:var(--primary)">
            ...
            <td style="padding:13px 18px;font-size:20px;font-weight:800;color:#fff">{{ rupiah($totalPendapatan) }}</td>
            <td></td>
          </tr>
        </tfoot>
      @endif
